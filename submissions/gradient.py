...
def _build_assemble_ctx(
    ...
    n_hard = int(benchmark.num_hard_macros)
    n_macros = int(benchmark.num_macros)
    orig_full = torch.zeros(n_macros, 2, device=device, dtype=dtype)
    if n_hard > 0:
        orig_full[:n_hard] = benchmark.macro_positions[:n_hard].to(
    ...
    return orig_full, movable_idx_t, n_hard, n_macros


def _assemble_full_fast(
    pos_hard: torch.Tensor,
    pos_soft: torch.Tensor,
    ctx: tuple[torch.Tensor, torch.Tensor, int, int],
) -> torch.Tensor:
    """Hoisted-constant equivalent of ``_assemble_full`` (no Python per-macro loop)."""
    orig_full, movable_idx_t, n_hard, n_macros = ctx
    out = orig_full.clone()
    if pos_hard.numel() > 0:
...
class GradientPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        clamp_ctx = _build_clamp_ctx(
            benchmark, assemble_ctx[1], n_hard, cw, ch, device, grad_dtype
        )
