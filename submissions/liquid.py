...
Schedule (default **2** cycles, env ``MACRO_PLACE_LIQUID_CYCLES``):

  Before each cycle, record PLC **proxy_cost** and placement snapshot. Then **liquid → patience**:

    1. **Liquid GPU** (explore) — GPU **surrogate** until plateau (no PLC): every **50** epochs,
       loss must improve by at least **0.001 ×** surrogate total at **phase start**;
       **3** consecutive sub-threshold checks stop explore (epoch cap **20000**). ``lr=1e-2``;
       ``w_density`` / ``w_overlap`` ``0.8`` / ``0.5``.
    2. **Patience GPU** (refine) — Adam, **PLC proxy** every **50** epochs, scale-only ``w·surrogate`` calibration
       (WL/density/congestion, no ``k``) after checkpoints, min-abs stagnation **0.0001** per
       proxy check vs session **best**;
       **4** consecutive sub-threshold checks stop patience. Epoch cap **20000** (or env).
...
Env:
    MACRO_PLACE_LIQUID_CYCLES — max liquid→patience pairs (default ``2``)
    MACRO_PLACE_LIQUID_GPU_EPOCHS — explore phase **max** epochs (default ``20000``); set ``0`` to skip.
    MACRO_PLACE_LIQUID_SURROGATE_MIN_REL_INITIAL — min improvement as fraction of initial surrogate
       loss at explore start (default ``0.001``)
    MACRO_PLACE_LIQUID_SURROGATE_STAG_PATIENCE — consecutive sub-threshold surrogate checks (default ``3``)
    MACRO_PLACE_LIQUID_SURROGATE_CHECK_EVERY — explore surrogate check interval (default ``50``)
    ...
    MACRO_PLACE_LIQUID_STAGNATION_PATIENCE — consecutive sub-threshold checks before stop (default ``4``)
...
_DEFAULT_LIQUID_SURR_STAG_PATIENCE = 3
...
@contextmanager
def _liquid_gpu_env():
    """Explore pass: no PLC proxy checks (surrogate plateau only)."""
    ...
    try:
        ...
        old = saved["MACRO_PLACE_GPU_PROXY_CHECK_EVERY"]
        if old is None:
            os.environ.pop("MACRO_PLACE_GPU_PROXY_CHECK_EVERY", None)
        else:
            os.environ["MACRO_PLACE_GPU_PROXY_CHECK_EVERY"] = old
...
class LiquidPlacer:
    """
    (Surrogate explore → PLC patience) × cycles, per-cycle PLC gate → Abbacus.
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        congestion_md = (_ROOT / "logs" / f"liquid_{_bench_slug(benchmark.name)}_congestion.md").resolve()
        ...
        with _default_cuda_for_gpu_phase():
            for cycle in range(cycles_cap):
                ...
                if liquid_epoch_cap <= 0:
                    ...
                    print(
                        f"[liquid] cycle {cycle + 1}/{cycles_cap} phase=liquid "
                        f"(surrogate plateau: min_improve={liquid_surr_rel:g}×initial_loss "
        ...
        print(f"[liquid] completed {cycles_done} cycle(s)", flush=True)
        ...
        print(f"[liquid] wrote congestion report {rel_congestion_md}", flush=True)
